...
def _make_pair(d_src: int, d_tgt: int, n: int, seed: int) -> tuple[torch.Tensor, torch.Tensor]:
    ...
    true_A = torch.randn(d_src, d_tgt)
    target = source @ true_A + 0.02 * torch.randn(n, d_tgt)
    return source, target


def test_procrustes_recovers_orthogonal_map():
    torch.manual_seed(0)
    source = torch.randn(200, 10)
    R_true = torch.randn(10, 10)
    q, _ = torch.linalg.qr(R_true)
    target = source @ q
    R, scale = procrustes_map(source, target)
    assert torch.allclose(R @ R.mT, torch.eye(10), atol=1e-8)
...
def test_affine_map_matches_target_with_bias():
    torch.manual_seed(2)
    source = torch.randn(150, 6)
    W_true = torch.randn(6, 4)
    bias = torch.randn(4)
